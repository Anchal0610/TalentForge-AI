...
export default function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  const handleGoogleCallback = async (code: string) => {
    ...
    try {
      ...
      // Log them in via context locally
      await login(data.user.email);

      // Clean the code parameter from the URL
      window.history.replaceState({}, document.title, window.location.pathname);
    } catch (err) {
      console.error('Google authentication exchange failed:', err);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errData = await res.json();
        throw new Error(errData.error || 'Failed to initialize Google Login');
      }
      const data = await res.json();
      if (data.url) {
  ...
  // Hydration and OAuth Callback check
        ...
        <CardContent className="space-y-4 px-4 pb-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button
              ...
              variant="outline"
              className="w-full border-zinc-850 text-white hover:bg-zinc-900 font-semibold h-10 text-xs uppercase tracking-wider transition-all duration-300 rounded-lg cursor-pointer flex items-center justify-center gap-2"
            >
              {googleLoading ? (
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />
              ) : (
